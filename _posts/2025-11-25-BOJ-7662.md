...
1. [두 개의 `PriorityQueue`를 이용한 `Lazy Deletion` 방식](#풀이-1-priorityqueue)
2. [`TreeMap`에 값의 등장 횟수를 저장해 멀티셋(`Multiset`)처럼 활용하는 방식](#풀이-2-treemap)

<br>

## 풀이 1: PriorityQueue
`PriorityQueue`를 사용해 이중 우선순위 큐를 구현합니다.
...
## 풀이 2: TreeMap
`TreeMap`을 사용해 이중 우선순위 큐를 구현합니다.
