...
tags: [Kotlin, 백트래킹]
description: 1부터 N까지의 자연수 중에서 M개를 오름차순으로 선택해 만드는 모든 조합을 백트래킹으로 구하는 Kotlin 완전 탐색 문제입니다.
---
...
## 풀이
`1`부터 `N`까지의 자연수 중에서 `M`개를 선택해 <span class="txt_bg">오름차순 수열</span>을 만드는 조합 문제입니다.

백트래킹으로 길이가 `M`인 수열을 채워 나가며,\
이전에 선택한 숫자보다 큰 수만 이어서 선택하도록 시작값을 넘겨줍니다.
