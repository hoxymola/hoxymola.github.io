...
## 유니온 파인드
<span class="txt_bg">유니온 파인드(Union-Find)란</span>?\
두 개의 연산을 통해 서로 겹치지 않는 여러 집합을 관리하는 자료구조입니다.
- **Union**: 두 원소를 같은 집합으로 합친다.
- **Find**: 해당 원소가 속한 집합을 찾는다.

## 경로 압축 (Path Compression)
![path compression.png](../assets/1043/path%20compression.png)

트리로 집합을 구현하면 트리가 한쪽으로 치우칠 수 있습니다.\
<span class="txt_bg">경로 압축</span>은 `find` 연산 중 거쳐간 노드들의 부모를 루트(🥔)로 바꿔,\
