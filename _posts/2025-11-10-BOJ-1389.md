...
description: 플로이드-워셜 알고리즘으로 모든 사람 간 최단 거리를 계산해 케빈 베이컨 수가 가장 작은 사람을 찾는 Kotlin 그래프 최단 경로 문제입니다.
---
...
플로이드-워셜 알고리즘은, 임의의 노드 `i`에서 `j`로 가는 최단 거리 `d[i][j]`를 계산하기 위해\
중간 노드 `k`를 경유하는 모든 경로 `d[i][k] + d[k][j]`를 고려합니다.
