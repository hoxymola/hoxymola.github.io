...
## 풀이
<span class="txt_bg">다익스트라(`Dijkstra`) 알고리즘</span>을 사용하면,\
가중치가 양수인 그래프에서 <span class="txt_bg">특정 시작점으로부터 모든 정점까지의 최단 거리</span>를 구할 수 있습니다.

우선순위 큐에 `(정점, 지금까지의 비용)`을 넣고,\
가장 비용이 작은 정점부터 꺼내며 최단 거리를 확정한 뒤, 인접한 정점들을 큐에 추가합니다.

가중치가 양수이기 때문에, 정점이 처음 큐에서 꺼내질 때의 거리가 반드시 해당 정점까지의 최단 거리입니다.\
